...
extern wstring GetIMEComposition();
extern string wstring_to_utf8(const std::wstring &wstr);
extern wstring utf8_to_wstring(const std::string &str);
extern void CancelIMEComposition();
const int vietnameseCodepoints[2523] = {
    ...
    0x30a1, 0x30a2, 0x30a3, 0x30a4, 0x30a5, 0x30a6, 0x30a7, 0x30a8, 0x30a9, 0x30aa, 0x30ab, 0x30ac, 0x30ad, 0x30ae, 0x30af, 0x30b0, 0x30b1, 0x30b2, 0x30b3, 0x30b4, 0x30b5, 0x30b6, 0x30b7, 0x30b8, 0x30b9, 0x30ba, 0x30bb, 0x30bc, 0x30bd, 0x30be, 0x30bf, 0x30c0, 0x30c1, 0x30c2, 0x30c3, 0x30c4, 0x30c5, 0x30c6, 0x30c7, 0x30c8, 0x30c9, 0x30ca, 0x30cb, 0x30cc, 0x30cd, 0x30ce, 0x30cf, 0x30d0, 0x30d1, 0x30d2, 0x30d3, 0x30d4, 0x30d5, 0x30d6, 0x30d7, 0x30d8, 0x30d9, 0x30da, 0x30db, 0x30dc, 0x30dd, 0x30de, 0x30df, 0x30e0, 0x30e1, 0x30e2, 0x30e3, 0x30e4, 0x30e5, 0x30e6, 0x30e7, 0x30e8, 0x30e9, 0x30ea, 0x30eb, 0x30ec, 0x30ed, 0x30ee, 0x30ef, 0x30f0, 0x30f1, 0x30f2, 0x30f3, 0x30f4, 0x30f5, 0x30f6, 0x30f7, 0x30f8, 0x30f9, 0x30fa, 0x30fb, 0x30fc, 0x30fd, 0x30fe, 0x30ff};
double hopChu::tGTamThoi = 0;
double hopChu::tGBatDauNhan = 0;

hopChu::hopChu(Rectangle hinhThaiDV, const string chuDV, Color mauNenDV, Color mauVienDV, Color mauChuDV, Font kieuChu) : hop(hinhThaiDV, mauNenDV, mauVienDV), mauChu(mauChuDV), kieuChu(kieuChu), chuCoDinh("")
{
    chu = utf8_to_wstring(chuDV);
...
void hopChu::cNoiDung(const string chuDV)
{
    chu = utf8_to_wstring(chuDV);
    soKiTuTruocConTro = chu.size();
}
...
void hopChu::bieuDien()
{
    ...
    hop::bieuDien();
    string chuHienThiDau = wstring_to_utf8(chu.substr(0, soKiTuTruocConTro));
    string chuHienThiCuoi = wstring_to_utf8(chu.substr(soKiTuTruocConTro, chu.size()));
    string chuoiDangNhap = wstring_to_utf8(chuoiIME);

    const float doDangCachChu = 1.0f;
    if (flagChuThayThe && !tTChon && chu.empty())
    {
        Vector2 kichCoChuCoDinh = MeasureTextEx(kieuChu, chuHienThiDau.c_str(), coChu, doDangCachChu);
        DrawTextEx(kieuChu, chuCoDinh.c_str(), (Vector2){hinhThai.x + this->doDayVien, hinhThai.y + this->doDayVien + (hinhThai.height - kichCoChuCoDinh.y) / 2}, coChu, doDangCachChu, mauChu);
    }

    Vector2 kichCoChuDau = MeasureTextEx(kieuChu, chuHienThiDau.c_str(), coChu, doDangCachChu);
    Vector2 kichCoChuDangNhap = MeasureTextEx(kieuChu, chuoiDangNhap.c_str(), coChu, doDangCachChu);

    DrawTextEx(kieuChu, chuHienThiDau.c_str(), (Vector2){hinhThai.x + this->doDayVien, hinhThai.y + this->doDayVien + (hinhThai.height - kichCoChuDau.y) / 2}, coChu, doDangCachChu, mauChu);
    DrawTextEx(kieuChu, chuoiDangNhap.c_str(), (Vector2){hinhThai.x + this->doDayVien + kichCoChuDau.x + doDangCachChu, hinhThai.y + this->doDayVien + (hinhThai.height - kichCoChuDau.y) / 2}, coChu, doDangCachChu, mauChu);
    DrawTextEx(kieuChu, chuHienThiCuoi.c_str(), (Vector2){hinhThai.x + this->doDayVien + kichCoChuDau.x + doDangCachChu * 2 + kichCoChuDangNhap.x, hinhThai.y + this->doDayVien + (hinhThai.height - kichCoChuDau.y) / 2}, coChu, doDangCachChu, mauChu);

    if (tTChon && int(GetTime() * 10) % 2 == 0)
        DrawTextEx(kieuChu, "|", (Vector2){viTriConTro.x - doDangCachChu, viTriConTro.y}, coChu, 0, mauChu);
}
void hopChu::capNhatTT()
{
    ...
    if (tTChon)
    {
        ...
        if (kyTuVuaNhan == 0)
            chuoiIME = strChuoiIME;
        strChuoiIME = GetIMEComposition();
        ...
        else if (kyTuVuaNhan != 0 && kyTuVuaNhan != '~' && !((((kyTuVuaNhan < '0' || kyTuVuaNhan > '9') && kyTuVuaNhan != '.' || (chu == L"" && kyTuVuaNhan == '.')) || chu.size() > 8) && chiNhapSo))
        {
            if (!chuoiIME.empty())
            {
                chu.insert(soKiTuTruocConTro, chuoiIME);
                soKiTuTruocConTro += chuoiIME.size();
                chu.insert(soKiTuTruocConTro, 1, L' ');
            }
            else
            {
    ...
    wstring wtrSubChu = chu.substr(0, soKiTuTruocConTro);
    kichThuocChu = MeasureTextEx(kieuChu, wstring_to_utf8(wtrSubChu + strChuoiIME).c_str(), coChu, 1.0f);
